...
def detect_outliers(numeric_columns):
    ...
    # The IQR method is commonly used to detect outliers because it effectively identifies extreme values
    # relative to the distribution of the data. It's simple and robust but may miss outliers in skewed distributions.
    return outliers

def calculate_skewness(numeric_columns):
    """Calculate skewness for numerical columns."""
    skewness_values = numeric_columns.skew().to_string()
    
    # Skewness is a measure of the asymmetry of the data. High skewness could indicate that data transformations 
    # like log or square root might be needed to normalize the data, especially for machine learning models.
...
def perform_pca(numeric_columns):
    ...
    # PCA helps reduce the dimensionality of the data while preserving as much variance as possible. 
    # It is particularly useful when working with high-dimensional datasets or when identifying principal 
    # components that explain variance, making data analysis more efficient.
...
def perform_clustering(numeric_columns):
    ...
    # K-Means clustering is used to group the data into clusters based on similarities. While basic, it can be 
    # effective for segmenting the dataset. However, more advanced clustering techniques (e.g., DBSCAN) could 
    # be explored for more complex or non-spherical clusters.
    return clusters
...
def generate_prompt(base_folder, column_types, summary_stats, missing_values, outliers, skewness_values, pca_variance, clusters, correlation_matrix=None):
    """Generate a dynamic prompt for LLM with clear integration of visualizations and explanation."""
    prompt = f"""
